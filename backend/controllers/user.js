...
const register = async (req, res) => {
    ...
    if(user){
        ...
        await User.create({first_name, last_name, gender, email, password: hash});
...
const login = async (req, res) => {
    ...
    if(user){
        ...
        if(is_match){
            const jwt_token = await jwt.sign({id: user._id}, process.env.JWT_SECRET, {expiresIn: "7d"});
            return res.json({status: "success", token: jwt_token, user: user});
        }else{
...
const user_update = async (req, res) => {
    ...
    await User.findByIdAndUpdate(id, {first_name, last_name, email, gender});
