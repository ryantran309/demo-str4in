...
function Profile() {
    ...
    useEffect(() => {
        axios.post(`${SERVER_URI}/user/user_data`, {token: localStorage.getItem("token")}).then((res) => {
            setID(res.data._id);
    ...
    function update_user(e){
        e.preventDefault();
        axios.post(`${SERVER_URI}/user/user_update`, {
            id: id,
    ...
    function password_modifier(e){
        e.preventDefault();
        axios.post(`${SERVER_URI}/user/password_modifier`, {
            id: id,
            password: password,
        ...
        <div className="card card-body" style={{borderRadius: "20px"}}>
            <div className="d-flex justify-content-between">
                ...
                <Link to="/"><i className="fas fa-times"></i></Link>
                        ...
                        <div className="alert alert-success alert-dismissible fade show" role="alert">user info has been updated<button type="button" className="close" data-dismiss="alert"><span aria-hidden="true">&times;</span></button></div>
                        </div>
                        <div className="col-6">
                        ...
                        <div className="input-group">
                            ...
                            {(gender === "Female")?
                            <select className="custom-select" id="gender" onChange={gender_selector}>
                                <option value="Female" defaultValue id="g_female">Female</option>
                                <option value="Male" id="g_male">Male</option>
                            </select>
                            :
                            <select className="custom-select" id="gender" onChange={gender_selector}>
                                <option value="Male" defaultValue id="g_male">Male</option>
                                <option value="Female" id="g_female">Female</option>
                            </select>}
                        </div>
                        </div>
                        <div className="col-12">
                            <small data-toggle="modal" data-target="#password_modal" style={{cursor: "pointer"}}><i className="fas fa-lock text-danger"></i> Password Change</small>
                        </div>
                        <hr className="mt-0 w-25 mx-auto" />
                        <div className="col-12 text-center">
                            <button type="button" className="btn btn-success btn-sm" onClick={update_user}>Save Changes</button>
                        </div>
                        </div>
        </div>
        </div>
...
<div class="modal fade" id="password_modal" tabIndex="-1" role="dialog">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div className="d-none" id="old_password_error"><small className="text-danger font-weight-bold">old password doesn't match!</small></div>
        <label htmlFor="old_password">Old Password</label>
        <input type="text" className="form-control" id="old_password" onChange={(e) => setOldPassword(e.target.value)} value={oldPassword} />
      ...
      <div class="modal-footer d-flex justify-content-between p-0">
        ...
        <button type="button" class="btn btn-primary btn-sm" onClick={password_modifier}>Change</button>
      </div>
    </div>
  </div>
</div>
