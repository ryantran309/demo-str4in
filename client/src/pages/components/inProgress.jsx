...
function InProgress() {
    ...
    useEffect(() => {
        axios.get(`${SERVER_URI}/todo/workspace_todos`, {params:{status: "inProgress", workspace: localStorage.getItem("ws")}}).then((res) => {
    ...
    return (
        ...
        <div className="col-md-9" id="todoDropzone_ip">
            <h5 className="d-flex justify-content-between align-items-center font-weight-bold"><i className="fas fa-circle" style={{fontSize: "13px", color: "#ffc93c"}}></i> In Progress <span className="badge d-flex justify-content-center align-items-center" style={{backgroundColor: "#ffc93c", color: "#ffffff", fontSize: "13px", width: "20px", height: "20px", borderRadius: "50%"}}>{todos.length}</span></h5>
            <div className="d-flex flex-column" onDragOver={(e)=>onDragOver(e)} onDrop={(e)=>{onDrop(e, "inProgress")}}>
            {todos.map((task) => {
                return (
            ...
            <Link to="/add" state={{status: "inProgress", workspace: localStorage.getItem("ws")}} className="btn btn-lg px-4" style={{fontSize: "14px", color: "#ffffff", backgroundColor: "#5046e5", borderRadius: "10px"}}><i className="fas fa-plus fa-sm"></i> Add New Card</Link>
            </div>
            </div>
        </div>
        </div>
